...
function formatRoundHeader(fullRound) {
    return fullRound ?? '?';
}
...
function writeAnnotations(node, skipLineAfterCommentIfLong, pushToken, skipLine) {
    ...
    for (const tagKey of tags) {
        const tagValue = (0, common_1.trimAndCollapseSpaces)(node.tag(tagKey).replace(/[[\]]/g, '')); // Square-brackets are erased in tag values in PGN.
        if (tagValue) {
 ...
 */
function writeGame(game, options) {
    let result = '';
    // Mandatory tags
 ...
 */
function writeGames(games, options) {
    return games.map(game => writeGame(game, options)).join('\n\n');
}
//# sourceMappingURL=pgn_write_impl.js.map